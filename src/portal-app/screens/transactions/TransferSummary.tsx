...
/**
 * Post-submit landing screen for New Transfer Request — Figma's
 * "Transfer Request Summary + Transaction ID tracking" (nodes
 * 2893:9837-10473). Previously the flow only toasted the transaction ref
 * and redirected straight back to the hub with no dedicated confirmation
 * screen to review or reference later.
 */
export default function TransferSummary() {
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .select('id, status, transaction_ref, created_at, details')
    ...
    return (
      <AppShell>
        <p className="rounded-core-md border border-dashed border-core-border p-8 text-center text-core-text-muted">
          We couldn't find that transfer request.
        </p>
      </AppShell>
    )
  }
  ...
  return (
    <AppShell>
      <div className="mx-auto flex max-w-2xl flex-col items-center gap-6 py-8 text-center">
        ...
        <div>
          <h1 className="text-[22px] font-semibold text-core-text">Transfer Request Submitted</h1>
        ...
        <div className="w-full rounded-core-md border border-core-border bg-core-surface p-6 text-left">
          ...
          <div className="flex flex-col gap-2 pt-4 text-[14px]">
            ...
            <div className="flex justify-between">
              <span className="text-core-text-muted">Investment</span>
              <span className="font-medium text-core-text">{row.details.investment}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-core-text-muted">Transfer by</span>
              <span className="font-medium text-core-text">{row.details.transferBy}</span>
            </div>
            <div className="flex justify-between">
